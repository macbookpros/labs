        ...
        /* Get matrix size header */
          MPI_File_open(MPI_COMM_WORLD,A_file,MPI_MODE_RDONLY, MPI_INFO_NULL,&config.A_file);
            MPI_File_open(MPI_COMM_WORLD,B_file,MPI_MODE_RDONLY, MPI_INFO_NULL,&config.B_file);
             MPI_File_open(MPI_COMM_WORLD,outfile,MPI_MODE_WRONLY, MPI_INFO_NULL,&config.C_file);
   ...
   /* Sub div cart communicator to N row communicator */
       ...
       int starts[2]={config.my_row*(config.local_dims[0]),config.my_col*(config.local_dims[0])};
        ...
        /* Create data array to load actual block matrix data */
          ...
          config.A=(double*)malloc(config.local_dims[0] *config.local_dims[0] *sizeof(*config.A));
          config.B=(double*)malloc(config.local_dims[0] *config.local_dims[0]* sizeof(*config.B));
          config.C=(double*)malloc(config.local_dims[0] *config.local_dims[0]* sizeof(*config.C));
         int i;
...
void compute_fox()
{
     double *buff=(double*)malloc(config.local_dims[0]*config.local_dims[0] * sizeof(*buff));
       config.A_tmp=(double*)malloc(config.local_dims[0] *config.local_dims[0]* sizeof(*config.A_tmp));
        ...
        for (i = 0; i <config.local_dims[0]; i++)
                for (j = 0; j <config.local_dims[0]; j++)
                        for (k = 0; k <config.local_dims[0]; k++)
                                config.C[i*config.local_dims[0]+j] += config.A[i*config.local_dims[0]+k]*config.B[k*config.local_dims[0]+j];
...
void multiply_matrixtemp()
{
        ...
        for (i = 0; i <config.local_dims[0]; i++)
                for (j = 0; j <config.local_dims[0]; j++)
                        for (k = 0; k <config.local_dims[0]; k++)
                                config.C[i*config.local_dims[0]+j] += config.A_tmp[i*config.local_dims[0]+k]*config.B[k*config.local_dims[0]+j];
